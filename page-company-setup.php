<?php include('header.php'); ?>
<!--========== ▼ article　（ページ毎にid名を変更） ==========-->
<article id="company-setup" class="">

  <div class="s-page__head">
    <div class="s-page__head-inner">
      <a href="<?php echo esc_url( home_url() ); ?>" class="s-page__head-logo">
        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/common/logo_white.svg" alt="上辻会計事務所" width="173" height="86">
      </a>

      <div class="s-ttl s-page__head-ttl">
        <span class="icon-plus"></span>
        <h2 class="s-ttl__text s-font__en page-text__fade block --sp --first"><span class="fade-txt">Company</span></h2>
        <h2 class="s-ttl__text s-font__en page-text__fade block --sp"><span class="fade-txt">Setup</span></h2>
        <p class="s-ttl__desp js-page__ttl-sub --left --service">会社設立支援</p>
      </div>
    </div>

    <p class="s-page__head-txt">理想のビジネスを実現する、第一歩を、確実にサポートいたします。<br>
    会社設立に必要な手続きや計画をスムーズに進めるための専門的なアドバイスと実務支援を提供します。<br>
    会社設立の支援なら、私たちにお任せくださいませ。</p>
  </div>

  <!--========== ▼ main_content ==========-->
  <main class="main_content">

    <section class="s-about">
      <div class="s-section__inner">
        <div class="s-about__flex">
          <div class="s-about__item --txt">
            <div class="about-ttl">
              <div class="s-ttl --about">
                <span class="icon-plus"></span>
                <h2 class="s-ttl__text s-font__en js-fade__content-ttl">About</h2>
                <p class="s-ttl__desp js-fade__sec --left">サービス概要</p>
              </div>
            </div>
            <strong class="s-about__item-ttl">新しいビジネスをスタートされる方、<br class="none --sp">
            これから会社を立ち上げる方のために</strong>
            <p class="s-about__item-txt">新しいビジネスの第一歩は、確実な設立準備が第一ステップです。<br>
            私たちは、会社形態の選定から登記申請、税務・労務管理の整備まで、スムーズなスタートを全面的に支援します。<br>
            <br>
            さらに、助成金や融資の活用、事業成長を見据えた戦略的アドバイスを提供し、経営の安定と発展をサポート。煩雑な手続きを専門家が代行することで、クライアントが本業に集中できる環境を整え、確実な成功へと導きます。</p>

          </div>
          <picture class="s-about__item">
            <source media="(max-width: 768px)" srcset="<?php echo get_template_directory_uri(); ?>/assets/images/common/about_img_03_sp.png" type="image/png">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/common/about_img_03.png" class="s-about__item-img" alt="サービス概要">
          </picture>
        </div>
      </div>
    </section>

    <section class="s-service">
      <div class="s-section__inner">
        <div class="section-ttl">
          <div class="s-ttl --recruit">
            <span class="icon-plus"></span>
            <h2 class="s-ttl__text s-font__en js-fade__content-ttl">Service</h2>
            <p class="s-ttl__desp js-fade__sec --left">サービス詳細</p>
          </div>
        </div>

        <div class="s-service__flex">

          <div class="s-service__flex-item js-fade__bottom --bottom">
            <strong class="s-service__flex-ttl">設立手続きの代行</strong>
            <p class="s-service__flex-txt">登記申請や必要書類の作成を含む、煩雑な手続きをワンストップで対応いたします。</p>
          </div>

          <div class="s-service__flex-item js-fade__bottom --bottom">
            <strong class="s-service__flex-ttl">最適な会社形態の提案</strong>
            <p class="s-service__flex-txt">株式会社、合同会社（LLC）など事業内容に適した形態をアドバイスいたします。</p>
          </div>

          <div class="s-service__flex-item js-fade__bottom --bottom">
            <strong class="s-service__flex-ttl">定款作成・認証サポート</strong>
            <p class="s-service__flex-txt">法的要件を満たした定款の作成と、公証人の認証手続きまでをトータルで支援します。</p>
          </div>

          <div class="s-service__flex-item js-fade__bottom --bottom">
            <strong class="s-service__flex-ttl">税務・法務のサポート</strong>
            <p class="s-service__flex-txt">設立後の税務申告や労務管理に向けた準備をサポートし、スムーズな運営を実現します。</p>
          </div>

          <div class="s-service__flex-item js-fade__bottom --bottom">
            <strong class="s-service__flex-ttl">資金調達のアドバイス</strong>
            <p class="s-service__flex-txt">設立時の資金計画や助成金、融資制度の活用についてご提案。</p>
          </div>
        </div>

        <p class="s-service__txt">事業のスタートアップを円滑に進めるため、豊富な経験と知識を活かして万全のサポートをお約束します。<br>
会社設立なら、私たちにお任せくださいませ。</p>
      </div>
    </section>




    <section class="s-page__link --company-setup">
      <div class="s-section__inner js-fade__bottom --bottom">
        <div class="s-page__link-flex">
          <div class="s-page__flex-item --img">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/common/kamitsuji.png" alt="代表　上辻 恭介">
            <p>代表 <span>上辻 恭介</span></p>
          </div>
          <div class="s-page__flex-item --txt">
            <strong class="s-page__item-ttl">私たちと共に、成功のカギとなる確実な準備を</strong>
            <p class="s-page__item-txt">会社設立は事業の未来を左右する重要なステップです。適切な形態選定や手続きを進めることで、スムーズなスタートが可能になります。専門家の支援を活用し、本業に集中できる環境を整えることが成功への近道です。私たちと共に、確実に準備をしていきましょう。</p>

            <div class="s-page__flex-btn">
              <div class="b-button --orange s-page__btn">
                <a class="b-button__link" href="<?php echo esc_url( home_url() ); ?>/price">料金表はこちら
                  <div class="b-button__link-arrow">
                    <span class="arrow"></span>
                  </div>
                </a>
              </div>
              <div class="b-button --orange s-page__btn">
                <a class="b-button__link" href="<?php echo esc_url( home_url() ); ?>/service">サービス一覧を見る
                  <div class="b-button__link-arrow">
                    <span class="arrow"></span>
                  </div>
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>

    <?php include('components/c-contact.php'); ?>
  </main>
  <!--========== ▲ main_content ==========-->

</article>
<!--========== ▲ article　（ページ毎にid名を変更） ==========-->
<?php include('footer.php'); ?>